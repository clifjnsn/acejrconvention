...
if($_GET['schoolid']) {
  $result=mysqli_query($mysql_handle, "select * from convention_schools where id =".(int)$_GET['schoolid']."  order by name");
} else {
  $result=mysqli_query($mysql_handle, "select * from convention_schools order by name");
}
...
//  $pdf->SetFillColor(0,0,0);
  ...
  while($row2 = mysqli_fetch_assoc($result2))
  {
    ...
    $result3 .= " convention_scores.studentid= ".(int)$row2['id']." and convention_event.id = convention_scores.eventid order by convention_event.id";
    ...
    while($row3 = mysqli_fetch_assoc($events))
    {
      ...
      $pdf->SetX(60);
      if($row3['team']) {
        $pdf->Cell(20,6,$row3['eventid']." - ".$row3["name"]." (".$row3['team'].") ",0,0,'L',0);
      } else {
        $pdf->Cell(20,6,$row3['eventid']." - ".$row3["name"],0,0,'L',0);
      }
      $pdf->SetX(160);
      $pdf->Cell(20,6,$row3["time"],0,1,'L',0);
    }
  }
}
